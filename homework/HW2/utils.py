...
def save_loss_comparison_by_dataset(results: dict[tuple[int, str], ResultsDict]):
    ...
    for h in hidden_sizes:
        groups[h] = [
            (h, d) for h, d in configs if h == hidden_sizes[0] or h == hidden_sizes[1]
        ]
    ...
    plt.figure()
    fig, ax = uks_mpl.subplots(2, 2, width_per=5, height_per=4)
...
def save_loss_comparison_by_hidden(results: dict[tuple[int, str], ResultsDict]):
    ...
    ax[0][0].plot(
        range(len(l1["train_loss"])), l1["train_loss"], label="hid_size=" + str(h1)
    )
    ax[0][0].plot(
        range(len(l2["train_loss"])), l2["train_loss"], label="hid_size=" + str(h2)
    )
    ax[0][0].title.set_text("Train Loss | Dataset = " + d1)

    ax[0][1].plot(
        range(len(l1["val_loss"])), l1["val_loss"], label="hid_size=" + str(h1)
    )
    ax[0][1].plot(
        range(len(l2["val_loss"])), l2["val_loss"], label="hid_size=" + str(h2)
    )
    ax[0][1].title.set_text("Val Loss | Dataset = " + d1)

    ax[1][0].plot(
        range(len(l3["train_loss"])), l3["train_loss"], label="hid_size=" + str(h1)
    )
    ax[1][0].plot(
        range(len(l4["train_loss"])), l4["train_loss"], label="hid_size=" + str(h2)
    )
    ax[1][0].title.set_text("Train Loss | Dataset = " + d2)

    ax[1][1].plot(
        range(len(l3["val_loss"])), l3["val_loss"], label="hid_size=" + str(h1)
    )
    ax[1][1].plot(
        range(len(l4["val_loss"])), l4["val_loss"], label="hid_size=" + str(h2)
    )
...
def plot_scaling_law(
    ...
    for parameters, results in all_results.items():
        ...
        # Plot the trajectory
        plt.plot(
            flops, val_loss, marker="o", label=f"params={parameters}", linewidth=2.5
        )
...
def plot_scaling_law_poly(all_results: dict[int, TrainerHistDict]):
    ...
    for parameters, results in all_results.items():
        ...
        plt.plot(
            flops_smooth, val_loss_smooth, label=f"params={parameters}", linewidth=2.5
        )
...
def plot_isoflop(
    ...
    for idx, (target, (parameters, val_loss), opt) in enumerate(
        zip(target_flops, params_vs_loss, optimal_params)
    ):
        if len(parameters) > 2:
